...
Функция принимает список транзакций (список словарей) и возвращает итератор, который фильтрует транзакции по валюте.
...
**Возвращает:**
- iterator — отфильтрованные транзакции


### transaction_descriptions

Генератор, который возвращает описание каждой транзакции по запросу.

**Параметры:**
- transactions (list[dict]) — список транзакций

**Возвращает:**
- generator — строки с описаниями транзакций


### card_number_generator

Генератор, который создаёт номера банковских карт в заданном диапазоне.

**Параметры:**
- start (int) — начальный номер
- stop (int) — конечный номер

**Возвращает:**
- generator — строки с номерами карт в формате XXXX XXXX XXXX XXXX
...
Проект поддерживает чтение финансовых операций из:
...
Добавлена поддержка:
- поиска транзакций с помощью регулярных выражений
- подсчета категорий операций
- пользовательского интерфейса через main.py
